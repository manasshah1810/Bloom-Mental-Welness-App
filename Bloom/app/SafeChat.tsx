...
import { 
  View, Text, StyleSheet, TouchableOpacity, ScrollView, TextInput, KeyboardAvoidingView, Platform, Modal
} from 'react-native';
import { useRouter, Link } from 'expo-router';
import { Feather, MaterialCommunityIcons } from '@expo/vector-icons';
import * as Haptics from 'expo-haptics';
import { ResizeMode, Video } from 'expo-av'; // Import the Video component

// --- IMPORTANT: SECURITY WARNING ---
const GEMINI_API_KEY = "AIzaSyBwTt1fasqUbhrjoAjTR-1kBqnvKebWjtE"; // As requested for prototype

// Define action types for messages
type MessageAction = 'play_breathing_video';

// Updated static responses and actions
const STATIC_RESPONSES: { [key: string]: { text: string, action?: MessageAction } } = {
...
// --- (ChatNavbar remains the same) ---
const ChatNavbar = () => (
  <View style={styles.navBar}>
    <Link href="/home" asChild><TouchableOpacity style={styles.navButton}><Feather name="home" size={28} color="#A9A9A9" /><Text style={styles.navText}>Home</Text></TouchableOpacity></Link>
    <TouchableOpacity style={styles.navButton}><Feather name="message-square" size={28} color="#C870FF" /><Text style={[styles.navText, { color: '#C870FF' }]}>Chat</Text></TouchableOpacity>
  </View>
);
...
export default function SafeChatScreen() {
  ...
  const [videoUri, setVideoUri] = useState<any>(null); // State to control video player
  const scrollViewRef = useRef<ScrollView>(null);
  const videoPlayerRef = useRef<Video>(null);

  useEffect(() => {
    setMessages([{ id: 'ai-welcome', text: "Hello! I'm here to listen. How are you feeling today?", sender: 'ai' }]);
  }, []);

  useEffect(() => {
    scrollViewRef.current?.scrollToEnd({ animated: true });
    // Check for an action in the latest AI message
    const lastMessage = messages[messages.length - 1];
    if (lastMessage?.sender === 'ai' && lastMessage.action === 'play_breathing_video') {
      // Use a timeout to allow the message to render before showing the video
      setTimeout(() => {
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={Platform.OS === "ios" ? 0 : 20}>
      ...
      {/* --- Video Player Modal --- */}
      <Modal visible={videoUri !== null} transparent={true} animationType="fade">
      ...
      <View style={styles.chatOptionsContainer}>
        {["I am feeling stressed", "I am feeling lonely", "I am feeling angry"].map((q) => (
          <TouchableOpacity key={q} style={styles.quickOptionButton} onPress={() => handleSendMessage(q)}>
      ...
      <View style={styles.inputContainer}>
        <TextInput style={styles.textInput} placeholder="Or write how you're feeling..." placeholderTextColor="#A9A9A9" value={inputText} onChangeText={setInputText} multiline />
...
const styles = StyleSheet.create({
  ...
  chatOptionsContainer: { flexDirection: 'column', paddingHorizontal: 20, paddingVertical: 15, borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.1)', borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.1)' },
  quickOptionButton: { backgroundColor: 'rgba(160, 80, 240, 0.2)', borderRadius: 25, paddingVertical: 15, paddingHorizontal: 20, marginBottom: 10, borderWidth: 1, borderColor: '#A050F0', alignItems: 'center' },
  ...
  userBubble: { alignSelf: 'flex-end', backgroundColor: '#A050F0', borderBottomRightRadius: 5 },
  aiBubble: { alignSelf: 'flex-start', backgroundColor: '#1F1A2A', borderBottomLeftRadius: 5, borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' },
  userText: { color: '#FFFFFF', fontFamily: 'Lato-Regular' },
  aiText: { color: '#E0B0FF', fontFamily: 'Lato-Regular' },
  inputContainer: { flexDirection: 'row', alignItems: 'center', padding: 15, borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.1)' },
  textInput: { flex: 1, backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 25, paddingHorizontal: 15, paddingVertical: 12, color: '#FFFFFF', fontFamily: 'Lato-Regular', fontSize: 16, marginRight: 10, minHeight: 48 },
  sendButton: { backgroundColor: '#A050F0', borderRadius: 25, width: 48, height: 48, justifyContent: 'center', alignItems: 'center' },
  navBar: { width: '100%', height: 90, backgroundColor: '#1F1A2A', borderTopLeftRadius: 25, borderTopRightRadius: 25, flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', paddingBottom: 10 },
  navButton: { alignItems: 'center' },
  navText: { fontSize: 12, fontFamily: 'Lato-Regular', marginTop: 4, color: '#A9A9A9' },
  // Video Player Styles
  videoOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.85)', justifyContent: 'center', alignItems: 'center' },
  videoPlayer: { position: 'absolute', top: 0, left: 0, bottom: 0, right: 0 },
  closeVideoButton: { position: 'absolute', top: 60, right: 20, backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: 20, padding: 5 },
});
