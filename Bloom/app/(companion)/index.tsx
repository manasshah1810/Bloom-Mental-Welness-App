    ...
    <Link href="/(companion)/" asChild>
      <TouchableOpacity style={styles.navButton}>
        <Feather
          name="message-circle"
          size={28}
          color={activeScreen === "chat" ? "#C870FF" : "#A9A9A9"}
        />
        <Text
          style={[
            styles.navText,
    ...
    <Link href="/(companion)/transcripts" asChild>
      <TouchableOpacity style={styles.navButton}>
        <Feather
          name="list"
          size={28}
          color={activeScreen === "transcripts" ? "#C870FF" : "#A9A9A9"}
        />
        <Text
          style={[
            styles.navText,
...
export default function TrueCompanionChatScreen() {
  ...
  const handleSend = async () => {
    ...
    const userMessagesText = finalMessages
  ...
  const renderItem = ({ item }: { item: Message }) => (
    <View
      style={[
        styles.messageRow,
        item.role === "user" ? styles.myMessageRow : styles.otherMessageRow,
      ]}
    >
      {item.role === "model" && (
        <Image
          source={require("../../assets/images/companion-avatar.png")}
          style={styles.avatar}
        />
      )}
      <View
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <FlatList
          ...
          ListEmptyComponent={() => (
            <View style={styles.emptyContainer}>
              <Image
                source={require("../../assets/images/companion-avatar-large.png")}
                style={styles.largeAvatar}
              />
              <Text style={styles.emptyText}>
                Your friend is here to listen.{"\n"}What's on your mind today?
              </Text>
            </View>
          )}
          onContentSizeChange={() =>
        ...
        />
        {isLoading && (
          <ActivityIndicator style={styles.loadingIndicator} color="#A050F0" />
        )}
